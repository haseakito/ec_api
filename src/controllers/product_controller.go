...
func (pc ProductController) DeleteProductImage(c echo.Context) error {
	...
	res := pc.db.Find(&productImage, "id = ? AND product_id = ?", productImageId, productId)
...
Returns:
	...
	An error if any occurred during the execution of the function, nil otherwise.
*/
func (pc ProductController) CreateReview(c echo.Context) error {
	// Get product id from request
	productId := c.Param("id")

	// Get a store with store id
	var product models.Product
	res := pc.db.Take(&product, "id = ?", productId)
	...
	if res := pc.db.Create(&review); res.Error != nil {
		c.JSON(http.StatusInternalServerError, res.Error)
		return nil
	}
...
Returns:
	...
	An error if any occurred during the execution of the function, nil otherwise.
*/
func (pc ProductController) GetReviews(c echo.Context) error {
	// Get product id from request
	productId := c.Param("id")
	...
	res := pc.db.Where("product_id = ?", productId).Find(&reviews)
...
Returns:
	...
	An error if any occurred during the execution of the function, nil otherwise.
*/
func (pc ProductController) DeleteReview(c echo.Context) error {
	// Get product id from request
	productId := c.Param("id")
	...
	res := pc.db.Find(&review, "id = ? AND product_id = ?", reviewId, productId)
	...
	if res := pc.db.Delete(&review); res.Error != nil {
		c.JSON(http.StatusInternalServerError, res.Error)
		return nil
	}
